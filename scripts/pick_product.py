...
- 여러 키워드로 검색해서 카테고리별 가격 하한 이상인 것만 후보로 삼는다
  ...
  2026-08-31: 생활용품/홈키친 카테고리 추가(사용자 요청, 가격 상관없이) — 하한을
  0으로 둬서 저가 상품도 걸러지지 않게 함.
...
CATEGORY_KEYWORDS = [
    ...
    # 생활용품/홈키친 (2026-08-31 사용자 요청, 가격 상관없이 소싱)
    ("주방용품세트", 0), ("밀폐용기세트", 0), ("프라이팬세트", 0),
    ("도마세트", 0), ("식기건조대", 0), ("텀블러", 0),
]
