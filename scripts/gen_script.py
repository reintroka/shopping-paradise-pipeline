...
- ig_caption: 인스타그램 릴스용 캡션. x_post와는 완전히 별도로, 인스타그램 사용자가 읽기 편하도록
  **반드시 문단 사이를 실제 줄바꿈 두 번(\\n\\n)으로 구분**해서 다음 순서로 작성할 것 —
  ...
  3) CTA 두 문장: 먼저 "프로필 링크"를 언급하며 구매/확인 유도하는 문장 하나(링크 URL 자체는
     넣지 말 것), 그 다음 줄(\\n)에 **아래 문구를 토씨 하나 바꾸지 말고 그대로** 포함시킬 것
     (인스타그램 댓글에 '가격'이라고 달면 자동으로 DM이 발송되도록 연결된 문구이므로 절대
     다른 단어로 바꾸지 말 것): "댓글에 '가격'이라고 남겨주시면 DM으로 바로 알려드려요!"
